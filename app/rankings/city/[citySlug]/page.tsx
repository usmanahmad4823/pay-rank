...
export default function CityLeaderboardPage() {
  ...
  useEffect(() => {
    ...
    async function fetchCityLeaderboard() {
      setIsLoading(true);
      try {
        const res = await fetch(`/api/rankings/city/${citySlug}?page=${page}&timeframe=${timeframe}`);
        ...
        if (res.ok && data.items) {
          ...
          setCityData({
            ...
            provinceSlug: data.provinceSlug || 'punjab',
          });
        }
      } catch (err) {
        console.error('Error loading city leaderboard:', err);
      } finally {
        setIsLoading(false);
      }
    }
  ...
  const displayCityName = cityData?.city || citySlug.replace(/-/g, ' ');
  const displayProvince = cityData?.province || 'Punjab';
  const provinceSlug = cityData?.provinceSlug || 'punjab';
    ...
    <div className="min-h-screen flex flex-col bg-[#FAF9F6] text-stone-900 font-sans selection:bg-coral-100 selection:text-coral-900">
      ...
      <main className="flex-1 max-w-6xl w-full mx-auto px-4 sm:px-6 py-6 sm:py-10 space-y-6">
        ...
        <nav className="flex items-center gap-1.5 text-xs text-stone-500 font-medium">
          ...
          <Link
            href={`/rankings/province/${provinceSlug}`}
            className="hover:text-stone-900 transition-colors"
          >
            {displayProvince}
          </Link>
          <ChevronRight className="w-3.5 h-3.5 text-stone-300" />
          <span className="font-bold text-stone-900">{displayCityName}</span>
        </nav>
        ...
        <div className="bg-white p-6 sm:p-8 rounded-3xl border border-stone-200/90 shadow-apple-card flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div className="space-y-2">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-stone-100 text-stone-700 text-xs font-bold border border-stone-200/60 uppercase tracking-wider">
              ...
              <span>{displayProvince} Province</span>
            </div>

            <h1 className="font-heading font-extrabold text-2xl sm:text-4xl text-stone-900 tracking-tight">
              {displayCityName} <span className="text-coral-500">Leaderboard</span>
            </h1>

            <p className="text-xs sm:text-sm text-stone-500 leading-relaxed">
              Official live pay-to-rank directory for restaurants listed in <strong className="text-stone-800">{displayCityName}</strong>.
            </p>
          </div>
        ...
        <div className="flex items-center justify-between border-b border-stone-200/80 pb-3">
          ...
          >
            ...
            <span>List Restaurant in {displayCityName}</span>
          </button>
        </div>
        ...
        {!isLoading && items.length === 0 ? (
          <div className="text-center py-16 bg-white rounded-3xl border border-stone-200 p-8 space-y-4 max-w-lg mx-auto shadow-apple-card">
            ...
            <h3 className="font-heading font-extrabold text-lg text-stone-900">
              No restaurants listed in {displayCityName} yet — be the first!
            </h3>

            <p className="text-xs text-stone-500 leading-relaxed">
              Claim rank #1 in {displayCityName} today and hold exclusive top visibility for your city.
            </p>
            ...
            >
              ...
              <span>Claim #1 Position in {displayCityName}</span>
            </button>
          </div>
        ) : (
          <LeaderboardTable
